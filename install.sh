...
if [ ! -f "main.tf" ]; then
    print_message "Creating main.tf with S3 backend configuration"
    cat > main.tf <<EOF
provider "aws" {
  region = "us-west-2"
}
...
EOF
    print_message "main.tf created successfully"
else
    print_message "main.tf already exists, skipping creation"
fi
